...
module.exports = generators.Base.extend({
			...
			this.prompt(addPropertyConfirmPrompt, function(response) {
				if (response.addProperty) {
					this.prompt(addPropertyPrompts, function(addPropertyResponse) {
						var property = { name: addPropertyResponse.propertyName, type: addPropertyResponse.propertyType, accessModifier: addPropertyResponse.propertyAccessModifier }					
  ...
  writing: function () {
    ...
    this.fs.copyTpl(
      ...
      { classTitle: this.projectionName, namespace: this.namespace, propertiesString: propertiesString }
    );
  }
});